...
export async function livroCreate(req, res) {
  const { titulo, genero, paginas, preco, foto, sinopse, admin_id } = req.body

  if (!titulo || !genero || !paginas || !preco || !foto || !sinopse || !admin_id) {
  ...
  try {
    const livro = await Livro.create({
      titulo, genero, paginas, preco, foto, sinopse, admin_id
    })
...
export async function livroUpdate(req, res) {
  ...
  const { titulo, genero, paginas, preco, foto, sinopse } = req.body

  if (!titulo || !genero || !paginas || !preco || !foto || !sinopse) {
    res.status(400).json("Erro... Informe titulo, genero, duracao, preco, foto, sinopse do filme")
    return
  }
  ...
  try {
    const livro = await Livro.update({
      titulo, genero, paginas, preco, foto, sinopse
    },
      {
        where: { id }
      })
